...
public func signOutAccount(vc: UIViewController, completionHandler: @escaping(Bool) -> ()) {
    
    LocalStore.get(key: keyStores.token) { (token) in
        
        Service.Alamofire_JSON(vc: vc, urlString: "users/logout?token=" + token, method: "GET") { (data, error) in
                    ...
                    if status_code == 200 {
                        ...
                        DispatchQueue.main.async {
                            ...
                            LocalStore.removeAll { (state) in
                                LocalStore.getMulti(keys: [keyStores.token, keyStores.statusLogin]) { (store) in
                                    
                                    print(store[0].value ?? "nil")
                                    print(store[1].value ?? "nil")
                        ...
                        AlertIOS(vc: vc, title: NSLocalizedString("failed", comment: ""), message: desc_err)
                    }
                }
            }
        }
